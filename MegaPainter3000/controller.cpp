...
{
    this->sbH = sbH;
    QObject::connect(this->sbH, SIGNAL(valueChanged(int)), this, SLOT(changeHue(int)));
    this->sbS = sbS;
    QObject::connect(this->sbS, SIGNAL(valueChanged(int)), this, SLOT(changeSaturation(int)));
    this->sbBR = sbBR;
    QObject::connect(this->sbBR, SIGNAL(valueChanged(int)), this, SLOT(changeValue(int)));

}

void Controller:: setSliderHSB(QSlider *sH, QSlider*sS, QSlider*sBR)
{
    this->sH = sH;
    QObject::connect(this->sH, SIGNAL(valueChanged(int)), this, SLOT(changeHue(int)));
    this->sS = sS;
    QObject::connect(this->sS, SIGNAL(valueChanged(int)), this, SLOT(changeSaturation(int)));
    this->sBR = sBR;
    QObject::connect(this->sBR, SIGNAL(valueChanged(int)), this, SLOT(changeValue(int)));
...
void Controller:: updateColor()
{
   /* if (pen->color()!= Qt::white && pen->color()!= Qt::black)*/ colorStyleSl.setHsl(pen->color().hslHue(),pen->color().hslSaturation(),127);
    ...
    //Заблокирую все сигналы
    sbR->blockSignals(true);
    sbG->blockSignals(true);
    sbB->blockSignals(true);
    sR->blockSignals(true);
    sG->blockSignals(true);
    sB->blockSignals(true);
    sbH->blockSignals(true);
    sbS->blockSignals(true);
    sbBR->blockSignals(true);
    slColor->blockSignals(true);
    ...
    sbBR->setValue(pen->color().value());
    sH->setValue(pen->color().hsvHue());
    sS->setValue(pen->color().hsvSaturation());
    sBR->setValue(pen->color().value());
    slColor->setValue(pen->color().lightness());
    // Установлю стиль вертикальному слайдеру
    ...
    //Разблокирую сигналы
    sbR->blockSignals(false);
    sbG->blockSignals(false);
    sbB->blockSignals(false);
    sR->blockSignals(false);
    sG->blockSignals(false);
    sB->blockSignals(false);
    sbH->blockSignals(false);
    sbS->blockSignals(false);
    sbBR->blockSignals(false);
    slColor->blockSignals(false);
...
void Controller:: changeHue(int hue)
{
    QColor c;
    c.setHsv(hue,pen->color().hsvSaturation(),pen->color().value());
    pen->setColor(c);
    updateColor();
}
void Controller:: changeSaturation(int saturation)
{
    QColor c;
    c.setHsv(pen->color().hsvHue(),saturation,pen->color().value());
    pen->setColor(c);
    updateColor();
}
void Controller:: changeValue(int value)
{
    QColor c;
    c.setHsv(pen->color().hsvHue(),pen->color().hsvSaturation(),value);
    pen->setColor(c);
    updateColor();
}
...
{
    QColor c;
    c.setHsl(colorStyleSl.hslHue(),colorStyleSl.hslSaturation(),lightness);
    pen->setColor(c);
    updateColor();
}

void Controller:: setIntrument(int instrument)
{
//    plane->setInstrument(instrument);
//    switch (instrument) {
//        case instrument::Pen: tbInstrument.set
//    }
}
